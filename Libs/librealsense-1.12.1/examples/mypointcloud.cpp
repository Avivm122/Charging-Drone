...
int main() try
{
    ...
    glfwInit();
    GLFWwindow * win = glfwCreateWindow(1280, 960, "My Point Cloud", nullptr, nullptr);
    ...
    while(!glfwWindowShouldClose(win))
    {
        ...
        for(int dy=0; dy<depth_intrin.height; ++dy)
        {
            for(int dx=0; dx<depth_intrin.width; ++dx)
            {
                ...
                float distance = sqrt(pow(depth_point.x,2)+pow(depth_point.y,2)+pow(depth_point.z,2));
                //printf("(%f,%f,%f) and %f\n", depth_point.x, depth_point.y, depth_point.z, distance);
                rs::float3 color_point = depth_to_color.transform(depth_point);
                rs::float2 color_pixel = color_intrin.project(color_point);
//                printf("(%f,%f,%f)\n", color_point.x, color_point.y, color_point.z);
                ...
                else if(distance>=0 && distance <=onehalfmeters)
                {
                    glColor3ub(255, 255, 255);
                }
                else
